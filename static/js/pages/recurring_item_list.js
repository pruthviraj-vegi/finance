document.addEventListener("DOMContentLoaded", function () {
  ...
  const tableContainer = document.getElementById("recurringTableContainer");

  let currentSearch = "";
  let currentType = "";
  let currentCategory = "";
  let currentSort = "next_due_date";
  let currentPage = 1;
  ...
  function fetchData() {
    const tableBody = tableContainer.querySelector(".recurring-list");
    if (tableBody) {
      tableBody.style.opacity = "0.5";
    }

    const queryUrl = `/expense/recurring/fetch/?search=${encodeURIComponent(currentSearch)}&type=${currentType}&category=${currentCategory}&sort=${currentSort}&page=${currentPage}`;
    ...
    fetch(queryUrl, {
      ...
      .catch(error => {
        ...
        showError("An error occurred while loading recurring items.");
      });
  }
